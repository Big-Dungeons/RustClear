...
pub fn blocks_macro(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    let BlockEntries { entries } = parse_macro_input!(input as BlockEntries);

    let mut enum_variants: TokenStream = Default::default();
    let mut block_toughness: TokenStream = Default::default();

    let mut serialization: TokenStream = Default::default();
    let mut deserialization: TokenStream = Default::default();

    let mut rotate: TokenStream = Default::default();
    ...
    for (block_id, entry) in entries.iter().enumerate() {
        ...
        let mut fields_named: TokenStream = Default::default();
        // { field1, field2 }
        let mut fields_struct_pattern: TokenStream = Default::default();

        let mut entry_serialization: TokenStream = Default::default();
        let mut entry_deserialization: TokenStream = Default::default();

        let mut entry_rotate: TokenStream = Default::default();
        ...
        if let Some(block_variants) = &entry.variants {
            ...
            let mut variant_deserialization: TokenStream = Default::default();
