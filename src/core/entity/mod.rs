...
fn on_mob_move(
    ...
    for (entity, mob, transform, old_transform, riding) in mob_query.iter() {
        ...
        if let Some(new_chunk) = chunks.get_mut(chunk_position) {
            ...
            // no point sending movement packets when it wouldn't move
            if riding.is_none() {
...
impl Plugin for MobPlugin {
    fn build(&self, app: &mut App) {
        app
            ...
            // system for mobs, order:
            // Update:
            //      main ticking, usually setting position for example
            // PostUpdate:
            //      handles the mob after its moved, by writing packets and moving its chunk reference,
            //      so you shouldn't move it here
            // Last:
            //      Cleanup
